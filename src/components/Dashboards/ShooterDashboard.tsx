...
/**
 * ShooterDashboard
 * Shoot schedule from calendar, assigned tasks.
 */
import { useEffect, useState } from 'react';
import { useRouter } from 'next/navigation';
import { Camera, CheckSquare, Clock, FileText } from 'lucide-react';
import styles from './Dashboard.module.css';

export default function ShooterDashboard({ userName, userId }: { userName: string; userId: string }) {
    ...
    const getDeadlineBadge = (endDate: string) => {
        ...
        return { label: 'Scheduled', cls: styles.badgeGreen };
    };
        ...
        <div className={styles.dashboard}>
            <div className={styles.header}>
                <div className={styles.headerLeft}>
                    ...
                    <h1 className={styles.title}>
                        {userName} 🎬
                        <span className={`${styles.roleChip} ${styles.roleShooter}`}>Shooter</span>
                    </h1>
                </div>
            </div>
            ...
            <div className={styles.statsRow}>
                <div className={styles.statCard}>
                    ...
                    <span className={styles.statSub}>Pending shoots</span>
                </div>
                <div className={styles.statCard}>
            ...
            <div className={styles.section}>
                <div className={styles.sectionHeader}>
                    <span className={styles.sectionTitle}><Camera size={13} /> Quick Actions</span>
                </div>
                <div className={styles.quickLinks}>
                    <div className={styles.quickCard} onClick={() => router.push('/reminders')}>
                        <div className={styles.quickCardIcon} style={{ background: 'var(--tag-yellow)' }}>
                            <CheckSquare size={18} color="var(--tag-yellow-text)" />
                        </div>
                        <div className={styles.quickCardText}>
                            <span className={styles.quickCardTitle}>My Tasks</span>
                            <span className={styles.quickCardDesc}>Shooting assignments</span>
                        </div>
                    </div>
                    <div className={styles.quickCard} onClick={() => router.push('/search')}>
                        ...
                        <div className={styles.quickCardText}>
                            <span className={styles.quickCardTitle}>View Scripts</span>
                            <span className={styles.quickCardDesc}>Reference for shoots</span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div className={styles.section}>
                    ...
                    <div className={styles.listCard}>
                        ...
                        {!loading && myTasks.length === 0 && (
                            <div className={styles.emptyState}>No shoot tasks assigned 🎉</div>
                        )}
                        {myTasks.map(t => {
