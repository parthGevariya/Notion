...
/**
 * EditorDashboard
 * Edit queue from calendar, assigned tasks.
 */
import { useEffect, useState } from 'react';
import { useRouter } from 'next/navigation';
import { Scissors, CheckSquare, Clock, FileText } from 'lucide-react';
import styles from './Dashboard.module.css';

export default function EditorDashboard({ userName, userId }: { userName: string; userId: string }) {
    ...
    const getDeadlineBadge = (endDate: string) => {
        ...
        if (hours < 24) return { label: 'Due today', cls: styles.badgeYellow };
        return { label: 'In queue', cls: styles.badgeBlue };
    };
        ...
        <div className={styles.dashboard}>
            <div className={styles.header}>
                <div className={styles.headerLeft}>
                    <span className={styles.greeting}>Time to edit,</span>
                    <h1 className={styles.title}>
                        {userName} ✂️
                        <span className={`${styles.roleChip} ${styles.roleEditor}`}>Editor</span>
                    </h1>
                </div>
            </div>
            ...
            <div className={styles.statsRow}>
                <div className={styles.statCard}>
                    <span className={styles.statLabel}><Clock size={14} /> Edit Tasks</span>
                    <span className={styles.statValue}>{loading ? '—' : myTasks.length}</span>
                    <span className={styles.statSub}>In your queue</span>
                </div>
                <div className={styles.statCard}>
            ...
            <div className={styles.section}>
                <div className={styles.sectionHeader}>
                    <span className={styles.sectionTitle}><Scissors size={13} /> Quick Actions</span>
                </div>
                <div className={styles.quickLinks}>
                    <div className={styles.quickCard} onClick={() => router.push('/reminders')}>
                        <div className={styles.quickCardIcon} style={{ background: 'var(--tag-green)' }}>
                            <CheckSquare size={18} color="var(--tag-green-text)" />
                        </div>
                        <div className={styles.quickCardText}>
                            <span className={styles.quickCardTitle}>My Edit Tasks</span>
                            <span className={styles.quickCardDesc}>Editing assignments</span>
                        </div>
                    </div>
                    <div className={styles.quickCard} onClick={() => router.push('/search')}>
                        <div className={styles.quickCardIcon} style={{ background: 'var(--tag-blue)' }}>
                            <FileText size={18} color="var(--tag-blue-text)" />
                        </div>
                        <div className={styles.quickCardText}>
                            <span className={styles.quickCardTitle}>Script Reference</span>
                            <span className={styles.quickCardDesc}>View client scripts</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className={styles.twoCol}>
                <div className={styles.section}>
                    <div className={styles.sectionHeader}>
                        <span className={styles.sectionTitle}><CheckSquare size={13} /> My Edit Tasks</span>
                    ...
                    <div className={styles.listCard}>
                        ...
                        {!loading && myTasks.length === 0 && (
                            <div className={styles.emptyState}>No edit tasks! 🎉</div>
                        )}
                        {myTasks.map(t => {
